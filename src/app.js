...
function renderButtons(itembutton, buttonState) {
...
function renderIngredientsBoard(){
  for (var item in state) {
    ...
    for (var items1 of items) {
      if (item == items1.innerHTML) {
        if (state[item]) 
          items1.style.display = "inherit";
        else 
...
function renderPrice(){
  ...
  for (each in state) {
    if(state[each]==true){
      cost+=ingredients[each];
    }
  }
  console.log(cost);
  document.querySelector(".price-details").innerHTML="you have to pay: "+cost;
}